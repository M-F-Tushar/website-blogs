...
| F-02 Uncommitted hardening | **Closed; awaiting merge** | 7 grouped commits (`fix(db)`, `feat(types)`, `security`, `feat(site)`, `fix(content)`, `test`, `docs`) | The 2026-08-04 working tree was committed in logical units *before* any new change, so nothing was ever at risk again. All work pushed to the fork `Raj-Indra-Asura/website-blogs` and opened as **[PR #5](https://github.com/M-F-Tushar/website-blogs/pull/5)** against `M-F-Tushar/website-blogs` (the repo Vercel deploys from). | The fork-PR CI run and Vercel preview both show `action_required` / `Authorization required` — the repository owner must approve them. |
...
| F-06 Unverifiable integrations | **Closed (in-app visibility); live confirmation is an operator step** | `feat(admin): integration status panel…`, `feat(admin): probe Supabase keys…` | Server-only `getIntegrationStatuses()` reports Supabase (public/service role), Turnstile, throttle secret, trusted IP header, and Resend wiring for the running stage — presence and mode only, never values. The two Supabase rows additionally **exercise the configured key against the configured URL** (`/auth/v1/health`) and report `Key accepted by <host>` or `error: Key rejected by <host>`. Rendered on `/admin/dashboard`. Operator guide Task I defines the live production check. | e2e asserts both Supabase rows show `ok` / `Key accepted by` and that no key-shaped strings appear in the page. |
...
| R-03 | **Critical (latent outage), environmental** | The Vercel **Production** environment held the URL and keys of a *second* Supabase project, `lbfphnumpirpgdwfevkf` (same schema, **no content, no media**; it had been auto-paused and was resumed during the audit). The live deployment was still compiled against `ljljrvqlrleppyzzfysj` — confirmed by the URL literal and the `sb_publishable_…G2TEDD` key in the served bundle, which `ljlj` accepts and `lbfph` rejects with `401 Invalid API key`. Because Vercel applies env changes only on the next build, the site was healthy but **one redeploy away from a fail-closed outage** of every public page. | Operator switched Production back to `ljljrvqlrleppyzzfysj` (URL, publishable key, secret key) the same day. Recommended: assign `lbfph…` to the **Preview** scope so it becomes staging (`VERCEL_ENV=preview` → `staging` stage). The Integrations panel key probe (F-06) now makes this class of mismatch visible on the dashboard immediately after deploy. |
...
1. **Approve and merge [PR #5](https://github.com/M-F-Tushar/website-blogs/pull/5).** As a fork PR it needs the owner to (a) approve the GitHub Actions run (`verify`, `smoke`, `e2e`) and (b) authorize the Vercel preview. Before merging, confirm in Vercel that the Production scope's three Supabase variables all belong to `ljljrvqlrleppyzzfysj`. Merging deploys to production.
2. **Post-deploy, read-only checks** (the auditor can run these on request): `curl -I https://tusherblog.me/` shows CSP/XFO/HSTS; `/feed.xml` is `application/rss+xml`; an unknown URL returns 404; anonymous `/admin/dashboard` returns 307; the served bundle still references `ljljrvqlrleppyzzfysj`.
3. **Post-deploy, owner only**: log in to `/admin/dashboard` → Integrations shows stage `production` and every row `ok`, with both Supabase rows reading `Key accepted by ljljrvqlrleppyzzfysj.supabase.co`.
4. **Manual Task H** — link the CLI and repair the production migration ledger (`npm run db:check` → printed `migration repair` command → `db:check` again).
5. **Manual Task I** — one live contact submission through Turnstile → `/admin/messages` → notification email received.
6. **Re-enter tags** on production posts that were expected to carry them (R-01), then confirm `post_tags` is populated.
7. Optional: link `lbfphnumpirpgdwfevkf`, bring it to the 11 tracked migrations, and use it as the e2e/staging target.

Upon completion of items 1–5, the system rating moves from **OPERATIONAL — CONDITIONAL** to **PRODUCTION-CLEAN**.
